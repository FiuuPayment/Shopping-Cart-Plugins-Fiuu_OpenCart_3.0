...
class ControllerExtensionPaymentMolpay extends Controller {
    public function index() {
        ...
        $data['amount'] = str_replace(',', '',number_format($this->currency->format($order_info['total'], $order_info['currency_code'], $order_info['currency_value'], false),2));
        ...
        $data['js'] = $this->config->get('payment_molpay_type').'MOLPay/API/seamless/latest/js/MOLPay_seamless.deco.js';
    ...
    //cancel order when close credit card popup
    ...
    public function return_ipn() {
        ...
        ************************************************************/
        ...
        if (in_array($status, $successStatus))  // replace this line with if($status=='00') if only want to update status when success payment
