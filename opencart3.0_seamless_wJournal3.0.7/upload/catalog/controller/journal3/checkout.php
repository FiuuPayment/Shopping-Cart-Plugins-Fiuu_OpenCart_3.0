...
class ControllerJournal3Checkout extends \Journal3\Opencart\Controller {
	...
	public function index() {
		...
		$data['confirm_block'] = $this->renderView('journal3/checkout/confirm', array(
			...
			'privacy'         => $this->customer->isLogged() ? null : Arr::get($this->model_journal3_links->getInformation($this->config->get('config_account_id')), 'text'),
		...
		if ($this->checkCart()) {
			...
			if (Arr::get($this->request->get, 'confirm') === 'true') {
				...
				if (!$this->customer->isLogged()) {
					$error = array_replace($error, $this->validateAccount());
				...
				$error = array_replace($error, $this->validateAddress('payment'));

				if (!$this->session->data['same_address'] && $this->cart->hasShipping()) {
					$error = array_replace($error, $this->validateAddress('shipping'));
				}
	...
	private function getCheckoutData($data) {
		...
		return array(
			...
			'total'                 => sprintf($this->language->get('text_items'), $this->cart->countProducts() + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0), $this->journal3->currencyFormat($data['total'])),
	...
	private function validateAddress($type) {
		$error = array();
		...
		$country_info = $this->model_localisation_country->getCountry(Arr::get($this->session->data, 'order_data.payment_country_id'));
	...
	private function registerAccount($order_data) {
		...
		if ($this->journal3->isOC2()) {
			...
			if (!$this->session->data['same_address']) {
				$this->model_account_address->addAddress($shipping_address);
			}
		} else {
		...
		$this->model_journal3_order->editCustomerId($customer_id);
