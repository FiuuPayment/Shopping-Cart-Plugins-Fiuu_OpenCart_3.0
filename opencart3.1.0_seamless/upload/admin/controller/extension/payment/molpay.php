...
class ControllerExtensionPaymentMolpay extends Controller {
    ...
    public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            // Trim whitespace from merchant credentials before saving
        ...
        // Load all channel labels from language file
        ...
        // Load per-channel enabled status from POST or saved config
        foreach ($data['channel_list'] as $key => $val) {
            if (isset($this->request->post['payment_molpay_'.$key.'_status'])) {
