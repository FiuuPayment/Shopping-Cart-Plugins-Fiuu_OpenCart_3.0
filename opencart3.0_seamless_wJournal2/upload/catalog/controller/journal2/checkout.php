...
//        $this->model_journal2_checkout->save();
        ...
        //for molpay process
        if ($order_data['payment_code'] == 'molpay')
        {

            $channel = $_POST['payment_options'];

            if ($channel == '')
                $errors['payment_channel'] = "Please select payment channel";
            $amount = $total;
            $data['status'] = true;
            $data['mpsmerchantid'] = $this->config->get('payment_molpay_mid');
            $data['mpschannel'] = $channel;
            $data['mpsamount'] = $total;
            $data['mpsorderid'] = $order_data['order_id'];
            $data['mpsbill_name'] = $order_data['firstname']. ' ' .$order_data['lastname'];
            $data['mpsbill_email'] = $order_data['email'];
            $data['mpsbill_mobile'] = $order_data['telephone'];
            $data['mpsbill_desc'] = $channel;
            $data['mpsvcode'] = md5($amount.$this->config->get('payment_molpay_mid').$this->session->data['order_id'].$this->config->get('payment_molpay_vkey'));
            $data['mpscurrency'] = $order_data['currency_code'];
        }
